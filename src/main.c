 ...
 * @author Massin KHELOUFI (35%) & Ilyes AGHOUILES (35%) & Imane ABDELBASSIR (30%)
 * @date 2025
 *
 * Ce fichier contient l'implémentation principale du gestionnaire de fichiers.
 * Il gère l'interface utilisateur en ligne de commande et coordonne toutes les
 ...
 */
 ...
 /** Nom de la partition courante */
 char *PARTITION_NAME;
 /** ID du répertoire courant */
 short currentDirectoryID = 0;
 /** Chemin courant dans la partition */
 char currentPath[MAX_FILES_NAME_SIZE * 10] = "";
 
 /**
  * @brief Affiche le message d'aide avec toutes les commandes disponibles
  */
 void showHelpMessage();
 
 /**
  * @brief Gère la création d'un nouveau fichier
  */
 void handleCreateFile();
 
 /**
  * @brief Gère le renommage d'un fichier existant
  */
 void handleRenameFile();
 
 /**
  * @brief Gère la lecture du contenu d'un fichier
  */
 void handleReadFile();
 
 /**
  * @brief Gère la modification du contenu d'un fichier
  */
 void handleModifyFile();
 
 /**
  * @brief Gère le formatage de la partition
  */
 void handleFormatPartition();
 
 /**
  * @brief Gère la visualisation de la structure de la partition
  */
 void handleVisualizePartition();
 
 /**
  * @brief Gère la création d'un nouveau dossier
  */
 void handleCreateFolder();
 
 /**
  * @brief Gère la suppression d'un dossier
  */
 void handleDeleteFolder();
 
 /**
  * @brief Gère le renommage d'un dossier
  */
 ...
 /**
  * @brief Gère la création d'un lien symbolique
  */
 void handleCreateSymbolicLink();
 
 /**
  * @brief Gère la création d'un lien physique
  */
 void handleCreateHardLink();
 
 /**
  * @brief Gère la lecture d'un lien symbolique
  */
 void handleReadLink();
 
 /**
  * @brief Gère le changement de répertoire courant
  */
 void handleChangeDirectory();
 
 /**
  * @brief Gère la modification des permissions d'un fichier
  */
 void handleChmod();
 
 /**
  * @brief Gère la sauvegarde de la partition
  */
 void handleBackupPartition();
 
 /**
  * @brief Gère la restauration d'une partition depuis une sauvegarde
  * @param backup_name Nom du fichier de sauvegarde
  */
